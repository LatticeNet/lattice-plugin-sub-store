...
watch(
  host.init,
  (value) => {
    if (value) void loadEndpointStatus();
  },
  { immediate: true },
);
...
const canCheck = computed(() => host.available(BINDINGS.importStatus));
const canImport = computed(() => host.available(BINDINGS.importRun));
const readyForAction = computed(
  () => !!endpointState.value.value && !collectionError.value && !busy.value && !host.bootError.value,
);
...
async function checkStatus(): Promise<void> {
  ...
  error.value = "";
  try {
    const request = callMethod<StatusResponse>(bridge, BINDINGS.importStatus, {
...
async function runImport(): Promise<void> {
  ...
  result.value = undefined;
  try {
    const request = callMethod<ImportResponse>(bridge, BINDINGS.importRun, {
      base_url: endpointState.value.value,
      sub_name: collection.value.trim(),
...
// ── preview (design-15 §7) ────────────────────────────────────────────────────
const preview = ref<ImportPreviewResponse>();
const previewing = ref(false);
const canPreview = computed(() => host.available(BINDINGS.importPreview));
...
async function runPreview(): Promise<void> {
  ...
  preview.value = undefined;
  try {
    preview.value = await callMethod<ImportPreviewResponse>(bridge, BINDINGS.importPreview, {
      base_url: endpointState.value.value,
      sub_name: collection.value.trim(),
      user_id: userId.value.trim() || undefined,
    }).promise;
...
const canManageEndpoint = computed(() => host.available(BINDINGS.endpointSave) && host.available(BINDINGS.endpointClear));
const usingSavedRef = computed(() => endpoint.value.trim().startsWith("secret://"));

async function loadEndpointStatus(): Promise<void> {
  if (!bridge || !host.available(BINDINGS.endpointStatus)) return;
  try {
    endpointStatus.value = await callMethod<EndpointStatusResponse>(bridge, BINDINGS.endpointStatus, {}).promise;
...
async function saveEndpoint(): Promise<void> {
  ...
  error.value = "";
  try {
    await callMethod(bridge, BINDINGS.endpointSave, {
      base_url: endpointState.value.value,
...
async function clearEndpoint(): Promise<void> {
  ...
  error.value = "";
  try {
    await callMethod(bridge, BINDINGS.endpointClear, {}).promise;
  ...
  <section>
    ...
    <section class="operation" aria-labelledby="import-title">
      ...
      <div class="operation-action">
        <span v-if="host.init.value && !canImport" class="permission-note">
