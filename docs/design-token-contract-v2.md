...
The emission check. `--radius-sm/md/lg/xl` and `--font-mono` live inside the `@theme inline { }` block (app.css:14-83). Under `inline`, Tailwind v4 bakes the value into each utility it generates (`.rounded-sm { border-radius: 3px }`, `.font-mono { font-family: ui-monospace, ... }`) instead of writing `var(--name)`, and the custom property itself reaches the compiled `:root` only when some stylesheet in the build references it with `var()`. Utility usage therefore says nothing: `rounded-sm/md/lg/xl` and `font-mono` are used in 12 / 61 / 31 / 6 / 50 files of `lattice-dashboard/src`, and that count is irrelevant to whether the property exists on `<html>`. What decides it is a `var()` reference, and the two groups differ there: `var(--radius-sm)` appears in the host (SshGuardView.vue:1631) and `var(--font-mono)` appears nowhere in `lattice-dashboard/src`. An existing dashboard build (the `clipboard-host` worktree, dist of 2026-09-03) shows the split: `--radius-sm:3px` is present in the compiled `:root`, `--font-mono:` is absent from every dist CSS file, and the mono stack exists only inside the `.font-mono` utility. That is not a value the bridge can read, so today `--font-mono` would cross to every plugin as an empty string, the alias `--lt-mono` would fall to its fallback, and nothing would visibly break, which is the failure mode that hides longest.

Because the two groups are emitted independently, one probe per group is the minimum, and one probe for both is wrong. The Gate 2 step for the dashboard half is two reads in the built console: `getComputedStyle(document.documentElement).getPropertyValue("--radius-sm")` for the radius group, and the same call for `--font-mono` for the type group. Each answer moves only its own group: an empty string for `--radius-sm` duplicates the four radius steps into the plain `:root` block next to `--radius`, and an empty string for `--font-mono` duplicates the mono stack there. Within the radius group one representative is enough, since the four steps sit in the same block and share the same emission path, so a build that emits `--radius-sm` emits its siblings the same way. On the evidence above, the expected result of the check is that the radius group passes and `--font-mono` fails, and the `:root` copy of `--font-mono` is added as part of the dashboard half rather than left to the check. The duplicated declarations in `:root` are the plain kind every other published token already uses; the `@theme inline` declarations stay, because the utilities still read them.
